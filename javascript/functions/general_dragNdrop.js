...
function addDragNdrop(dragSelector, dropSelector, scope, onSuccessfulDrop, onStartDrag, onStopDrag) {
	...
	*/
	...
	$(dragSelector).unbind('mousedown.myDrag');
	$(dragSelector).bind('mousedown.myDrag', function(event) {
		...
		copy = $(this).clone().addClass('beingDragged').width($(this).width()).offset({ top: pos.top, left: pos.left }).appendTo(scope);
		
		$(scope).bind('mousemove.trackDrag', function(event) {
			$(copy).offset({ top: event.pageY + elementBeingDraggedMouseOffset.y, left: event.pageX + elementBeingDraggedMouseOffset.x });
		});

		$(dropSelector).bind('mouseenter.myDrop', function(event) {
		...
		$(dropSelector).bind('mouseleave.myDrop', function(event) {
			if (elementBeingHovered === this) {
				elementBeingHovered = null;
				$(this).removeClass('hover');
			}
		});
		$(document).bind('mouseup.myDrop', function(event) {
			var success = false;
			if (elementBeingDragged && elementBeingHovered && ($(elementBeingHovered).hasClass(dropSelector.substring(1)) || $(elementBeingHovered).attr('id') == dropSelector.substring(1))) {
				...
				scriptError("Tried mouseup on a elementBeingDragged that was === null. This shouldn't happen");
				$(copy).remove();
				copy = null;
			}
			$(scope).css('cursor', 'auto');
			$(scope).unbind('mousemove.trackDrag');
			$(document).unbind('mouseup.myDrop');
			$(dropSelector).unbind('mouseenter.myDrop');
			$(dropSelector).unbind('mouseleave.myDrop');
